...
def run(world_size: int, rank: int, local_rank: int, graph_dataset,sample_names,label_encoder):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        print(f"Rank={rank}, train_data:{len(train_dataset)}, test_data:{len(train_dataset)}")
        ...
        # Initialize model
        model = SlideGNN_AdvancedMIL(input_dim=1024, hidden_dim=512, num_classes=4,num_layers=3, dropout=0.3).to(local_rank)
        ...
        for epoch in range(1, 101):
            train_loss = train(model, train_loader, optimizer, criterion, device=local_rank)
            dist.barrier()
            slide_accuracy, slide_score, slide_preds, slide_labels, tile_score, tile_preds, tile_batch = test(model,test_loader, device=local_rank)
            dist.barrier()
            if rank == 0:
        ...
        results_path= os.path.join(fold_dir, f"predicted_results_rank{rank}.txt")
        save_results_to_text(results_path, sample_names, val_idx, slide_labels, slide_preds, slide_score, label_encoder)
            
         # Save to an .npz file
         np.savez(os.path.join(fold_dir,f"tile_results{rank}.npz"),
